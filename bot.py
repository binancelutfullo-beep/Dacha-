...
TEXTS = {
    "uz": {
        ...
        "ask_owner_name_uz": "1/6. Dacha nomini o'zbek tilida kiriting (masalan: 🏡 Mening dacham):",
        "ask_owner_name_ru": "2/6. Endi nomini rus tilida kiriting:",
        "ask_owner_desc_uz": "3/6. Tavsifni o'zbek tilida kiriting (xonalar soni, qulayliklar va h.k.):",
        "ask_owner_desc_ru": "4/6. Tavsifni rus tilida kiriting:",
        "ask_price": "5/6. Bir kechalik narxni faqat raqamlarda kiriting (masalan: 800000):",
        "invalid_price": "❌ Narx faqat raqamlardan iborat bo'lishi kerak. Masalan: 800000",
        "ask_photo": "6/6. Endi dacha rasmini yuboring (rasm sifatida, fayl emas):",
    ...
    "ru": {
        ...
        "ask_owner_name_uz": "1/6. Введите название дачи на узбекском (например: 🏡 Моя дача):",
        "ask_owner_name_ru": "2/6. Теперь введите название на русском:",
        "ask_owner_desc_uz": "3/6. Введите описание на узбекском (кол-во комнат, удобства и т.д.):",
        "ask_owner_desc_ru": "4/6. Введите описание на русском:",
        "ask_price": "5/6. Введите цену за ночь только цифрами (например: 800000):",
        "invalid_price": "❌ Цена должна состоять только из цифр. Например: 800000",
        "ask_photo": "6/6. Теперь отправьте фото дачи (именно как фото, не как файл):",
...
A = {
    ...
    "back": "⬅️ Orqaga",
    "ask_name_uz": "1/6. Dacha nomini o'zbek tilida kiriting (masalan: 🏡 Chimyon dachasi):",
    "ask_name_ru": "2/6. Endi nomini rus tilida kiriting:",
    "ask_desc_uz": "3/6. Tavsifni o'zbek tilida kiriting:",
    "ask_desc_ru": "4/6. Tavsifni rus tilida kiriting:",
    "ask_price": "5/6. Bir kechalik narxni faqat raqamlarda kiriting (masalan: 800000):",
    "invalid_price": "❌ Narx faqat raqamlardan iborat bo'lishi kerak. Masalan: 800000",
    "ask_photo": "6/6. Endi dacha rasmini yuboring (rasm sifatida, fayl emas):",
...
def main_menu_kb(lang: str, user_id: int) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton(text=t["menu_support"], callback_data="menu_support")],
    ]

    if rec and rec.get("status") == "approved":
        rows.append([InlineKeyboardButton(text=t["menu_owner"], callback_data="owner_panel_reopen")])
    elif not rec or rec.get("status") == "rejected":
        rows.append([InlineKeyboardButton(text=t["menu_owner_apply"], callback_data="owner_apply_start")])
...
def edit_field_kb(dacha_key: str, back_cb: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text=A["field_photo"], callback_data=f"ef_ph_{dacha_key}")],
...
FIELD_MAP = {"nu": "name_uz", "nr": "name_ru", "du": "desc_uz", "dr": "desc_ru", "pr": "price", "ph": "photo_id"}
...
async def show_dacha_detail(target, lang: str, dacha_key: str, d: dict):
    """Dacha tafsilotlarini ko'rsatadi — rasm bo'lsa rasm bilan, bo'lmasa oddiy matn bilan.
    ...
    kb = dacha_detail_kb(lang, dacha_key)
    ...
    if isinstance(target, CallbackQuery):
        ...
        if photo_id:
            await target.answer_photo(photo=photo_id, caption=text, reply_markup=kb)
        else:
...
async def support_write_save(message: Message, state: FSMContext, bot: Bot):
    ...
    await state.set_state(None)
    await message.answer(TEXTS[lang]["support_sent"], reply_markup=main_menu_kb(lang, message.from_user.id))
...
async def owner_apply_phone(message: Message, state: FSMContext, bot: Bot):
    ...
    await state.set_state(None)
    await message.answer(TEXTS[lang]["owner_apply_sent"], reply_markup=main_menu_kb(lang, message.from_user.id))
    ...
    if uid in owners:
        ...
        try:
            await bot.send_message(
                int(uid), TEXTS[u_lang]["owner_approved_msg"], reply_markup=main_menu_kb(u_lang, int(uid))
            )
        except Exception as e:
...
async def owner_reject(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if uid in owners:
        ...
        try:
            await bot.send_message(
                int(uid), TEXTS[u_lang]["owner_rejected_msg"], reply_markup=main_menu_kb(u_lang, int(uid))
            )
        except Exception as e:
            logging.warning(f"Foydalanuvchiga ({uid}) xabar yuborilmadi: {e}")
...
async def oa_price(message: Message, state: FSMContext):
    ...
    if price is None:
        ...
        return
    await state.update_data(o_price=price)
    await state.set_state(OwnerAddStates.photo)
    await message.answer(TEXTS[lang]["ask_photo"])


@router.message(OwnerAddStates.photo)
async def oa_photo(message: Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    lang = get_lang(data)
    ...
    try:
        pdf_path = generate_contract_pdf(message.from_user.full_name, data["o_name_uz"], data["o_price"])
...
async def admin_add_price(message: Message, state: FSMContext):
    ...
    await message.answer(A["ask_photo"])


@router.message(AdminStates.adding_photo)
