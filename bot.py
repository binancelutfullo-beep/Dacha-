"""
Dacha bron qilish Telegram bot + Admin panel + Qo'llab-quvvatlash
Til: O'zbek / Rus (foydalanuvchi uchun) | Admin panel: O'zbek
Kutubxona: aiogram 3.x
Ma'lumotlar: dachas.json, bookings.json, support.json, support_messages.json
"""
...
async def admin_reply_start(callback: CallbackQuery, state: FSMContext):
    ...
    if not ticket:
        ...
        return
    await state.update_data(reply_ticket_id=ticket_id, reply_page=page)
...
async def admin_reply_send(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        await bot.send_message(
            ticket["user_id"], TEXTS[user_lang]["support_reply_prefix"] + reply_text
        )
