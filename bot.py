...
A = {
    ...
    "owner_code_generated": "✅ Yangi kod yaratildi:\n\n`{code}`\n\n_(kodni bosing — avtomatik nusxalanadi)_\n\n📋 *Qanday ishlatiladi:*\n1. Ushbu kodni owner'ga o'zingiz yuboring (Telegram, SMS va h.k.)\n2. Owner botga /start bosadi\n3. Asosiy menyudan \"🏘 Dacha egasi bo'lish\" tugmasini bosadi\n4. \"🎟 Kodim bor\" tugmasini tanlaydi\n5. Shu kodni botga yuboradi\n6. Kod to'g'ri bo'lsa, u darhol tasdiqlangan dacha egasi bo'ladi\n\n⚠️ Kod faqat bir marta ishlatiladi.",
...
async def cancel_operation(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["operation_cancelled"])
...
async def menu_list(callback: CallbackQuery, state: FSMContext):
    ...
    if not dachas:
        ...
        return
    await callback.message.edit_text(TEXTS[lang]["choose_search_type"], reply_markup=search_menu_kb(lang))
    await callback.answer()
...
async def price_filter_menu(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    lang = get_lang(data)
    await callback.message.edit_text(TEXTS[lang]["choose_price_range"], reply_markup=price_filter_kb())
    await callback.answer()
...
async def search_name_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["ask_dacha_name"], reply_markup=cancel_kb(lang))
    await callback.answer()
...
async def guest_contract_agree(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["ask_checkin"], reply_markup=cancel_kb(lang))
    await callback.answer()
...
async def owner_apply_start(callback: CallbackQuery, state: FSMContext):
    ...
    if rec and rec.get("status") == "approved":
        ...
        await callback.answer()
        return
    await callback.message.edit_text(TEXTS[lang]["owner_entry_choice"], reply_markup=owner_entry_choice_kb(lang))
    await callback.answer()
...
async def owner_apply_form_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["ask_name"], reply_markup=cancel_kb(lang))
    await callback.answer()
...
async def owner_code_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["ask_owner_code"], reply_markup=cancel_kb(lang))
    await callback.answer()
...
async def contract_agree(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["ask_owner_name_uz"], reply_markup=cancel_kb(lang))
    await callback.answer()
...
async def oa_video_skip(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(OwnerAddStates.location)
    await callback.message.edit_text(TEXTS[lang]["ask_location"], reply_markup=cancel_kb(lang))
...
async def oa_district_selected(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await state.set_state(None)
    await callback.message.edit_text(TEXTS[lang]["dacha_added_owner"])
...
async def admin_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_name_uz"], reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def admin_video_skip(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminStates.adding_location)
    await callback.message.edit_text(A["ask_location"], reply_markup=cancel_kb("uz"))
...
async def admin_region_back(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer(A["not_admin"], show_alert=True)
        return
    await callback.message.edit_text(A["choose_region"], reply_markup=region_select_kb({}, "aregion", "admin_panel"))
    await callback.answer()
...
async def admin_district_selected(callback: CallbackQuery, state: FSMContext):
    ...
    save_dachas(dachas)
    await state.set_state(None)
    await callback.message.edit_text(A["added"])
...
async def admin_edit_ask_value(callback: CallbackQuery, state: FSMContext):
    ...
    if field == "photo_id":
        await callback.message.edit_text(A["ask_new_photo"], reply_markup=cancel_kb("uz"))
    else:
        await callback.message.edit_text(A["ask_new_value"].format(old=d[field]), reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def edit_media_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_photos"], reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def edit_media_photos_done(callback: CallbackQuery, state: FSMContext):
    ...
    if not data.get("edit_photos"):
        ...
        return
    await state.set_state(AdminStates.editing_video)
    await callback.message.edit_text(A["ask_video"], reply_markup=skip_inline_kb("uz", "editmedia_video_skip"))
    await callback.answer()
...
async def edit_district_selected(callback: CallbackQuery, state: FSMContext):
    ...
    save_dachas(dachas)
    await state.set_state(None)
    await callback.message.edit_text(A["updated"], reply_markup=_updated_manage_reply(callback.from_user.id, dachas))
    await callback.answer()
...
async def admin_contract_edit_ask(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_contract_text"].format(label=label, old=texts[kind]), reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def admin_owner_codes(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer(A["not_admin"], show_alert=True)
        return
    await callback.message.edit_text(A["owner_codes_panel_title"], reply_markup=owner_codes_menu_kb())
    await callback.answer()
...
async def owner_code_list(callback: CallbackQuery, state: FSMContext):
    ...
    codes = load_owner_codes()
    if not codes:
        await callback.message.edit_text(A["no_owner_codes"], reply_markup=owner_codes_menu_kb())
        await callback.answer()
        return
    rows = []
    ...
    await callback.message.edit_text(A["owner_codes_list_title"], reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await callback.answer()
...
async def admin_promocodes(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer(A["not_admin"], show_alert=True)
        return
    await callback.message.edit_text(A["promo_panel_title"], reply_markup=promocodes_menu_kb())
    await callback.answer()
...
async def promo_new_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_promo_code"], reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def promo_type_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(prompt, reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def promo_limit_skip(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_promo_expiry"], reply_markup=kb)
    await callback.answer()
...
async def promo_expiry_skip(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(None)
    await callback.message.edit_text(A["promo_created"].format(code=code), reply_markup=promocodes_menu_kb())
    await callback.answer()
...
async def promo_list(callback: CallbackQuery, state: FSMContext):
    ...
    promos = load_promocodes()
    if not promos:
        await callback.message.edit_text(A["no_promocodes"], reply_markup=promocodes_menu_kb())
        await callback.answer()
        return
    rows = []
    ...
    await callback.message.edit_text(A["promo_list_title"], reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await callback.answer()
...
async def admin_commission_edit_ask(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_commission"].format(name=owners[uid]["name"]), reply_markup=cancel_kb("uz"))
    await callback.answer()
...
async def admin_guests(callback: CallbackQuery, state: FSMContext):
    ...
    guests = load_guests()
    if not guests:
        await callback.message.edit_text(A["no_guests"], reply_markup=admin_panel_kb())
        await callback.answer()
        return
...
async def admin_support_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_support_title"], reply_markup=cancel_kb("uz"))
    await callback.answer()
