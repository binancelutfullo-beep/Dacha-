...
def main_reply_kb(lang: str, user_id: int) -> ReplyKeyboardMarkup:
    ...
    rows = [
        ...
        [KeyboardButton(text=t["about_btn"]), KeyboardButton(text=t["menu_travel"])],
    ]
...
@router.message(F.text.in_({TEXTS["uz"]["about_btn"], TEXTS["ru"]["about_btn"]}))
async def rk_menu_about(message: Message, state: FSMContext):
    data = await state.get_data()
    lang = get_lang(data)
    await state.set_state(None)
    await state.update_data(lang=lang)
    await message.answer(load_about_text())


@router.message(F.text.in_({TEXTS["uz"]["menu_travel"], TEXTS["ru"]["menu_travel"]}))
async def rk_menu_travel(message: Message, state: FSMContext):
    data = await state.get_data()
    lang = get_lang(data)
    await state.set_state(None)
    await state.update_data(lang=lang)
    await message.answer(TEXTS[lang]["coming_soon"])
