...
def _build_contract_pdf(filename_prefix: str, person_label: str, person_name: str, dacha_name: str,
    ...
    story = [
        ...
        Spacer(1, 10),
        Paragraph(f"Sana: {now.strftime('%d.%m.%Y %H:%M')}", styles["Normal"]),
        Paragraph(f"{person_label}: {person_name}", styles["Normal"]),
        Paragraph(f"Dacha nomi: {dacha_name}", styles["Normal"]),
        Paragraph(extra_line, styles["Normal"]),
        Spacer(1, 14),
    ...
    story.append(Paragraph("Elektron rozilik dalili:", styles["Heading2"]))
    consent = (
    ...
    story.append(Paragraph(consent, styles["Normal"]))
    doc.build(story)
    return path
...
async def show_dacha_detail(target, lang: str, dacha_key: str, d: dict):
    """Dacha tafsilotlarini ko'rsatadi — rasm/video albomi, lokatsiya va matn bilan.
    ...
    if isinstance(target, CallbackQuery):
        ...
        if has_media:
            ...
            if location:
                await bot.send_location(chat_id, latitude=location["lat"], longitude=location["lon"])
