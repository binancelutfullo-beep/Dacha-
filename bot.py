...
A = {
    ...
    "owner_code_generated": "✅ Yangi kod yaratildi:\n\n`{code}`\n\nBuni owner'ga o'zingiz yuboring (Telegram, SMS va h.k.). Kod bir marta ishlatiladi.",
...
def region_select_kb(dachas: dict, callback_prefix: str, back_cb: str) -> InlineKeyboardMarkup:
    """Faqat dachalar mavjud bo'lgan viloyatlarni ko'rsatadi (agar callback_prefix qo'shish
    oqimi uchun bo'lsa — bu holda hamma viloyat ko'rsatiladi, filtrsiz)."""
    if callback_prefix in ("oregion", "aregion"):
        present = set(REGIONS.keys())
    else:
        present = {d["region"] for d in dachas.values() if d.get("region")}
    rows = []
    if callback_prefix == "browseregion":
        rows.append([InlineKeyboardButton(text="💰 Narx bo'yicha qidirish", callback_data="pricefilter_menu")])
    rows += [
...
async def menu_list(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        TEXTS[lang]["choose_region"], reply_markup=region_select_kb(dachas, "browseregion", "back_main")
    )
    await callback.answer()
...
async def browse_region(callback: CallbackQuery, state: FSMContext):
    ...
    if region_key == "boshqa":
        ...
        if not filtered:
            ...
            return
        await callback.message.edit_text(
            TEXTS[lang]["choose_dacha"], reply_markup=dacha_list_kb(lang, filtered, "menu_list")
        )
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        ...
        reply_markup=district_select_kb(region_key, dachas, "browsedistrict", "menu_list"),
    )
    await callback.answer()
...
async def enter_phone(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=TEXTS[lang]["promo_skip_btn"], callback_data="promo_skip")]]
    )
...
async def owner_code_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(TEXTS[lang]["ask_owner_code"])
    await callback.answer()
...
async def edit_media_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_photos"])
    await callback.answer()
...
async def owner_code_generate(callback: CallbackQuery, state: FSMContext):
    ...
    save_owner_codes(codes)
    await callback.message.edit_text(A["owner_code_generated"].format(code=code), reply_markup=owner_codes_menu_kb())
    await callback.answer()
...
async def promo_new_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(A["ask_promo_code"])
    await callback.answer()
...
async def promo_value_entered(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=A["promo_skip_btn"], callback_data="promolimit_skip")]]
    )
...
async def promo_limit_skip(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=A["promo_skip_btn"], callback_data="promoexpiry_skip")]]
    )
